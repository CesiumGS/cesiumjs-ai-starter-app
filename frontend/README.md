...
There is no `frontend/.env` — `vite.config.ts` sets `envDir` to the repo root, so both `npm run dev` and `docker compose up` read the single top-level `.env` (see [`../.env.example`](https://github.com/CesiumGS/cesiumjs-ai-starter-app/blob/main/.env.example) and [top-level README § Environment Variables](https://github.com/CesiumGS/cesiumjs-ai-starter-app/blob/main/README.md#environment-variables)). Copy `../.env.example` to `../.env` and set `VITE_CESIUM_ION_ACCESS_TOKEN` (get one free at [ion.cesium.com](https://ion.cesium.com)). It's baked into the client bundle at build time — intentionally client-visible, so scope it in the Ion console. Optionally set `VITE_API_BASE_URL` to point at a non-default backend, or `VITE_OTEL_LOG_LEVEL` (`debug`/`info`/`warn`/`error`/`silent`) to control this app's browser-side logging (console + OTLP export alike) — defaults to `debug` in dev builds, `silent` in production (see `src/utils/config.ts`).
