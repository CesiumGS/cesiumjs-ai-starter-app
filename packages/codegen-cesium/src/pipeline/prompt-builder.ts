...
- Never use \`Cesium.IonResource\` or \`Cesium.Resource\` (e.g. \`Cesium.IonResource.fromAssetId(...)\` fed into another factory's \`.fromUrl(...)\`) — both are blocked from the execution sandbox for security reasons and are always \`undefined\` at runtime, so any snippet that references them fails immediately. When loading a Cesium ion asset by its numeric ID, call the target type's own \`fromIonAssetId\` factory directly instead — e.g. \`await Cesium.Cesium3DTileset.fromIonAssetId(assetId)\`, \`await Cesium.CesiumTerrainProvider.fromIonAssetId(assetId)\`, \`await Cesium.Cesium3DTilesTerrainProvider.fromIonAssetId(assetId)\` — never route through \`IonResource\`/\`Resource\` first.
...
- Never call \`.removeAll()\` (or read/write any other \`_\`-prefixed property, e.g. to "remember" whether you already did something across calls) on \`viewer.entities\`, \`viewer.dataSources\`, \`viewer.imageryLayers\`, \`scene.primitives\`, or \`scene.groundPrimitives\` to "start clean" before adding new content — it is always rejected, and the intent virtually never actually requires clearing existing scene content. Only call \`.removeAll()\` if the user's intent explicitly asks to remove/clear all existing entities, primitives, imagery, or data sources.
