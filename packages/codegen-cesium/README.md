...
directly into the codegen model's prompt. `logger` accepts
`@cesium-ai/observability`'s `Logger` and defaults to its silent `noopLogger`. It reports each
attempt's model-call failures, verification violations, and the overall success/failure once all
attempts are exhausted. The sample backend's `createExecuteCesiumCodeTool`
