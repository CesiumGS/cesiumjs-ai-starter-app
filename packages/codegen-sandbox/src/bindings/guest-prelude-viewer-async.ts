 ...
 * through a dedicated Asyncify bridge (`__cesiumSandboxHostCallAsync__`), on the assumption that
 ...
 * `viewer.*` property/method is now forwarded completely transparently. The same migration was
 * later applied to every remaining explicitly-bound `Cesium.*` factory
 * (`createWorldImageryAsync`, `Cesium3DTileset.fromUrl`, ...), which now also flow through the
 * generic bridge with no dedicated Asyncify mechanism left in the codebase at all.
 *
