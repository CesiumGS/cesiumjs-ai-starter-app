...
/**
 * Marker key identifying a JSON-safe, tagged encoding of a plain JS `Date` crossing the sandbox
 * boundary. Without this, a guest-constructed `Date` (e.g. `new Date("2026-07-20T00:00:00Z")`)
 * has no own enumerable properties, so the generic plain-object marshaling path silently
 * flattens it to `{}` on the way to the host — real CesiumJS APIs like `JulianDate.fromDate`
 * check `instanceof Date` and throw ("date must be a valid JavaScript Date") on that flattened
 * `{}`. Tagged with the value's ISO string instead, and revived back into a real `Date` at the
 * boundary (both host->guest and guest->host), the same way value types like `Cartesian3` are.
 */
...
export class SandboxHandles {
  ...
  private tagValueType(value: unknown): Record<string, unknown> | undefined {
    for (const definition of CESIUM_VALUE_TYPE_DEFINITIONS) {
      if (value instanceof definition.constructor) {
        const record = value as unknown as Record<string, unknown>;
        return Object.fromEntries([
          [VALUE_TYPE_MARK, definition.name],
          ...definition.fields.map((field) => [field, record[field]]),
        ]);
      }
    }
    return undefined;
  ...
  private reviveValueType(record: Record<string, unknown>): unknown {
    const name = record[VALUE_TYPE_MARK];
    const definition = CESIUM_VALUE_TYPE_DEFINITIONS.find((candidate) => candidate.name === name);
    if (definition) {
      return new definition.constructor(...definition.fields.map((field) => record[field]));
    }
    throw new Error(`Unknown tagged value type "${String(name)}"`);
  }
