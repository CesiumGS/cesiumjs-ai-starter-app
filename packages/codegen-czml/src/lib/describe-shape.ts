...
/**
 * Applies a `.describe()` hint to every field of a zod object shape. Generic over the shape so
 * it works for any tool's structural fields, not just one.
 */
...
/**
 * Builds a tool's **model-facing** input schema in one step: merges per-field `.describe()`
 * overrides over the tool's defaults ({@link mergeDescriptions}), then decorates the structural
 * shape with the merged text ({@link describeShape}).
 */
