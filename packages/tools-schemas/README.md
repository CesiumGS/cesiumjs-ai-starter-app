...
The model-facing catalogue currently contains 21 tools (`CESIUM_TOOL_NAMES`):
...
- **GeoJSON** — `geoJsonAdd`, `geoJsonRemove`
...
| `@cesium-ai/tools-schemas/json-schema` | `toolInputJsonSchema`                                                                              | Any consumer converting a tool's Zod input schema to a raw JSON Schema (e.g. `@cesium-ai/webmcp-cesium`, or a non-AI-SDK function-calling provider). Wraps `z.toJSONSchema()` and guards root-level `z.discriminatedUnion(...)` tools (`cameraOrbit`, `entityAdd`) so they still get a root `type: "object"` — call this instead of `z.toJSONSchema()` directly. |
