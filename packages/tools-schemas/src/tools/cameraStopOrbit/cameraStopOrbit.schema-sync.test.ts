...
import { defaultCameraStopOrbitInputSchema } from "./cameraStopOrbit.js";
...
const CASES: ReadonlyArray<{ name: string; input: unknown; valid: boolean }> = [
  ...
  { name: "unexpected extra field is ignored", input: { immediate: true }, valid: true },
  { name: "string input", input: "not-an-object", valid: false },
  { name: "null input", input: null, valid: false },
];

describe("cameraStopOrbit schema sync (frontend ⇄ backend)", () => {
  for (const { name, input, valid } of CASES) {
    test(`agree on "${name}"`, () => {
      const backend = defaultCameraStopOrbitInputSchema.safeParse(input).success;
      const frontend = cameraStopOrbitInputShape.safeParse(input).success;
