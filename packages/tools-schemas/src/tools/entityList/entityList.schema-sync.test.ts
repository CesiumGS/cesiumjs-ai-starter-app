...
import { defaultEntityListInputSchema } from "./entityList.js";
 ...
 * `buildEntityListInputSchema` instead of deriving them from the shared
...
const CASES: ReadonlyArray<{ name: string; input: unknown; valid: boolean }> = [
  ...
  { name: "extra field is ignored", input: { includeHidden: true }, valid: true },
  { name: "wrong type", input: null, valid: false },
];

describe("entityList schema sync (frontend ⇄ backend)", () => {
  for (const { name, input, valid } of CASES) {
    test(`agree on "${name}"`, () => {
      const backend = defaultEntityListInputSchema.safeParse(input).success;
      const frontend = entityListInputShape.safeParse(input).success;
