...
interface CreateMcpToolsOptions {
  ...
  logger?: Logger; // from @cesium-ai/observability; default: noopLogger
}
...
`@cesium-ai/observability` to enable console logging. `"info"` or louder is recommended in any
environment where you want to catch tool-poisoning-style changes—it logs every discovered tool's
name and description at connect time.
...
- Requires Node.js ≥ 22 (`@ai-sdk/mcp`'s own requirement), matching this repo's overall `>=22` floor.
