...
- `backend/src/mcp-app-router.ts` exposes `GET /api/mcp-app/resource` (reads a `ui://` resource via `@ai-sdk/mcp`'s `readMCPAppResource`) and `POST /api/mcp-app/tool-call` (validates the `(server, toolName)` pair against that request's own resolved tool set, then calls `MCPClient.callTool`).
- `packages/chat-element/src/McpAppWidget.tsx` renders the fetched HTML in a `sandbox="allow-scripts"` `srcDoc` iframe (never `allow-same-origin`, so the widget can never reach this app's cookies/DOM), injects a CSP `<meta>` tag built from the resource's own requested domains, and implements the host half of a `postMessage` bridge (`resources/read`, `ui/open-link`, and an approval-gated `tools/call`).
