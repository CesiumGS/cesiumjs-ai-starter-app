...
import { McpServerConfigsSchema } from "./types.js";

describe("McpServerConfigsSchema", () => {
  it("accepts a valid sse server config", () => {
    const parsed = McpServerConfigsSchema.parse([
  ...
  it("rejects an http transport with an invalid URL", () => {
    expect(() =>
      McpServerConfigsSchema.parse([
        { name: "weather", transport: { type: "http", url: "not-a-url" } },
      ]),
    ).toThrow();
  });
  ...
  it("rejects an unsupported (e.g. stdio) transport type", () => {
    expect(() =>
      McpServerConfigsSchema.parse([
        { name: "docs", transport: { type: "stdio", command: "node" } },
      ]),
    ).toThrow();
  });
